...
Given("Tengo el siguiente api: {string}", function (givenEndpoint: string) {
  this.endpoint = givenEndpoint;
});

When("Se busque por el usuario con id: {string}", function (givenId: string) {
  this.id = givenId;
});

Then(
  "El estado de la respuesta del api debe ser: {string}",
  async function (expectedStatusCode: string) {
